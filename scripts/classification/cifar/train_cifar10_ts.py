...
def main():
    ...
    if opt.teacher is not None and opt.hard_weight < 1.0:
        ...
        teacher = get_model(teacher_name, pretrained=True, classes=10, ctx=context)
        teacher.cast('float32')
        distillation = True
    else:
        distillation = False
    ...
    def train(epochs, ctx):
        ...
        if distillation:
            loss_fn = gcv.loss.DistillationSoftmaxCrossEntropyLoss(temperature=opt.temperature,
        ...
        for epoch in range(epochs):
            ...
            for i, batch in enumerate(train_data):
                ...
                if distillation:
                    teacher_prob = [nd.softmax(teacher(X.astype('float32', copy=False)) / opt.temperature) \
                                    for X in data]
                with ag.record():
                    output = [net(X) for X in data]
                    # loss = [loss_fn(yhat, y) for yhat, y in zip(output, label)]
                    if distillation:
                        # loss = [print(yhat.shape, y.shape, p.shape) for yhat, y, p in zip(output, label, teacher_prob)]
                        loss = [loss_fn(yhat, y, p) for yhat, y, p in zip(output, label, teacher_prob)]
                    else:
                        loss = [loss_fn(yhat, y) for yhat, y in zip(output, label)]
